                ...
                if (def.directives.findIndex(f => f.name.value == "json") > -1) {
                    ...
                    def.directives.find(f => f.name.value == "json").arguments.map(function (d) {
                        if (d.value.value.startsWith("Json")) {
                            ...
                            var jsonFields = JSON.parse("[" + json + "]")[0];
                            ...
                            if (typeof (jsonFields) == 'object') {
                                ...
                                field.json.push({ name: d.name.value.replace(/__/g, ":"), type: "Json", listType: true, nonNull: true, json: jsonObject })
                            } else {
                                field.json.push({ name: d.name.value.replace(/__/g, ":"), listType: true, type: jsonFields, nonNull: true })
                            }
