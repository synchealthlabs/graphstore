...
*/
export function isEnhancedObservableArray(thing: any): thing is IEnhancedObservableArray<any> {
 ...
 * @param thing the object to test
*/
export function isEnhancedObservable(thing: any): thing is IEnhancedObservableDelegate {
...
export function toJS_nonPrimary(source, primaryKeys: string[]) {
	...
	if (isEnhancedObservable(source)) {
		return untracked(() => Object.keys(source).concat(Object.keys(source["$emobx"])).filter(key => primaryKeys.indexOf(key) == -1).reduce((result, key, index, array) => { result[key] = toJS(source[key]); return result }, {}));
	}
...
export function toJS_Tracked(source) {
	...
	if (isEnhancedObservable(source)) {
		return Object.keys(source).concat(Object.keys(source["$emobx"])).reduce((result, key, index, array) => { result[key] = toJS(source[key]); return result }, {});
	}
