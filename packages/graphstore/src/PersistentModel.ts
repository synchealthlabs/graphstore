...
const {
  ...
  toJS_Tracked,
  makeNonEnumerable
} = PersistentModelObservable
import {
  observable as mobx_observable,
  reaction,
  action,
  untracked,
  runInAction
 ...
 */
export class Model
  implements PersistentModelObservable.IEnhancedObservableDelegate {
  private $emobx
  ...
  constructor(defaults: any, graphStore?: IGraphStore) {
    ...
    this._primaryKeys = Reflect.getMetadata(
      'primarykeys',
    ...
    makeNonEnumerable(this, 'loading', 'exists', 'status')
  }
  ...
  private inUse() {
    if (this._isDirty) {
      throw new Error('InUse: Founded database updates before first use')
    }
    ...
    if (self._source == SourceType.createFactory) {
      ...
      this._unsubscribe = this._graphStore.observeDocument(
        this._currentKeys,
        this._currentPath,
        this._primaryKeys,
        this
    ...
    this._dirtyStoreWatcherDispose = reaction(
      ...
      truthy => {
        ...
        if (self._primaryKeys.filter(key => data[key] == null).length > 0) {
          throw new Error(
            'All primary keys must be updated in a batch; at least one was null'
          )
        }
        ...
        self._graphStore.updateDocument(
          (<typeof Model>self.constructor).Store.path(self),
          this._primaryKeys,
          data,
          self.status,
          self._deleted
...
export class ModelCollection
  ...
  private _observableArray: PersistentModelObservable.IEnhancedObservableArray<
    any
  >
   ...
   */
  addObserver(): void {
    this._unsubscribe = this._graphStore.observeCollection(
      this._currentKeys,
      this._currentPath,
      this._primaryKeys,
...
export abstract class Store {
   ...
   */
  static path(model: any): string {
    /* must override */ throw new Error('Model class does not implement Store')
  }
}
