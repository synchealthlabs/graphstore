...
export interface IFireDatabaseRef {
  ...
  once(eventType: string): Promise<{ val: () => object }>
  off: () => void
  update(
    update_value: any,
  ...
  remove(callback: (a: Error) => void, isRemoteOrigin?: boolean)
}
...
export class GraphStore implements IGraphStore {
  ...
  public observeDocument(
    ...
    ref.on(
      'value',
      action((snapshot: IFireDatabaseSnapshot) => {
        if (snapshot == null) {
          model.exists = false
          model.loading = false
  ...
  public observeCollection(
    ...
    path: string,
    primaryKeys: string[],
    modelCollection: IObservableArray<any>,
    modelType: any
  ): () => void {
    let ref = this.db.ref(path)
    ...
    ref.on(
      ...
      action((snapshot: IFireDatabaseSnapshot) => {
        ...
        items.forEach(newitem => {
          let olditem = modelCollection.find(item =>
            primaryKeys.every(key => newitem[key] == item[key].toString())
          )
          if (olditem === undefined) {
            throw new Error('404 Database updated record that was not found')
          }

          Object.getOwnPropertyNames(newitem).forEach(key => {
    ...
    ref.on(
      ...
      action((snapshot: IFireDatabaseSnapshot) => {
        ...
        items.forEach(newitem => {
          ...
          if (olditem === undefined) {
            throw new Error('404 Database deleted record that was not found')
          }
          modelCollection.remove(olditem)
          olditem.keys().forEach(key => {
            this[key] = null
          })
  ...
  public updateDocument(
    ...
    primaryKeys: string[],
    update_value: any,
    status: IGraphStoreStatus,
    deleted: boolean
  ): void {
    status.executing = true

    if (
      Object.keys(update_value).length == 1 &&
      update_value.item_value !== undefined
    ) {
 ...
 */
function arrayObjecttToFlatArray(
  ...
  if (currentKey >= allPrimaryKeys.length) {
    throw new Error(
      'Tried to create collection when entire primary key is known'
    )
  }

  let nextPrimaryKey = allPrimaryKeys[currentKey]

  let preceedingKeys = Object['assign']({}, keys, {
    [nextPrimaryKey]: snapshot.key
  })
...
function flatMap(
  ...
  Object.keys(objectToMap).forEach(keyValue => {
    ...
    let _precedingKeys = Object['assign']({}, precedingKeys, {
      [keyName]: keyValue
    })
    flatMap(
      resultArray,
