...
export class GraphStoreMemory extends GraphStore {
    ...
    */
    constructor(initialState: any) {
        var db = new MemoryDatabase(initialState);
        super(db);
    }
}
...
export class MemoryDatabase implements IFireDatabase {
    ...
    */
    ...
    private _onCollection(path: string, childAdded: (data: any) => void): () => void {

        let self = this;

        self.subscriptionsCollection[path] = self.subscriptionsCollection[path] || [];
    ...
    private _update(path: string, update_value: any, deleted: boolean, callback: (a: Error) => void): void {
        ...
        if (deleted) {
            ...
            Object.keys(self.subscriptionsCollection).filter((subscriptionPath) => {
                ...
                if (subscriptionPath.length < path.length) {
                    ...
                    result = subsetFind(result, path.substr(subscriptionPath.length + 1));
                }
        ...
        Object.keys(self.subscriptions).filter((subscriptionPath) => {
...
function deepUpdate(obj: any, path: string, update_value: any): boolean {
    ...
    let paths = path.split('/')
        , current = obj
        , i, key, isAdded = false, parent = null;
