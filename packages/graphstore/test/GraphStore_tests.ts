...
DatabaseCases.forEach(db => {
  describe(db.constructor['name'], () => {
    ...
    it('should get collection of one item', done => {
      ...
      unsubscribe = db.observeCollection(
        keys,
        path,
        primaryKeys,
        modelCollection,
    ...
    it('should get collection of multiple item', done => {
      ...
      autorun(r => {
        ...
        if (!modelCollection['loading']) {
          expect(
            modelCollection.length >= 1 && modelCollection.length <= 2
          ).toBe(true)
    ...
    it('should get large collection of multiple items with multiple primary keys', done => {
      ...
      autorun(r => {
        ...
        if (!modelCollection['loading']) {
          expect(
            modelCollection.length >= 1 && modelCollection.length <= 22
          ).toBe(true)
          if (modelCollection.length == 22) {
            ...
            expect(model.loading).toBe(false)
            expect(model.exists).toBe(true)
            expect(model.userid_from).toBe(
              'google-Bk0ksad9asdadasdasdasdk0gggg:0'
            )
            expect(model.userid_to).toBe(
              'google-Jk0wcm5svSaBAU6Doh5UlPqBrZj1:0'
            )
            expect(model.id).toBe('-KflG_KP98yDWN0z6DIM')
            expect(model.encData).toMatch(
              'X3JrEw7ppT5eV3ySDCGrV/YzH0ltbMIlsj;1'
            )
            ...
            expect(model.encData).toMatch(
              'X3JrEw7ppT5eV3ySDCGrV/YzH0ltbMIlsj;5'
            )
            expect(model.signature).toMatch('9FbzCHf5CQt2qpFzmzI0nL7OiCYd8Msdf')
            r.dispose()
            unsubscribe()
      ...
      autorun(r => {
        ...
        if (!modelCollection['loading']) {
          ...
          if (modelCollection.length == 2) {
            ...
            expect(model.labels.device).toBe(
              'E62AD9DE-5814-4444-A212-AE928FD70558'
            )
            expect(model.labels.service).toBe(
              'life.besync.graphstore.staging.ios'
            )
            model = modelCollection[1]
            expect(model._deleted).toBe(false)
            expect(model._source).toBe(0)
    ...
    it('should get item with object-based primary keys and item_value', done => {
      var keys = {
        orgid: 'dunder-mifflin',
        userid: 'google-Jk0wcm5svSaBAU6Doh5UlPqBrZj1:0',
        role_id: 'consumer'
      }
      var primaryKeys = ['orgid', 'userid', 'role_id']
      var path =
        'orgUsers/dunder-mifflin/google-Jk0wcm5svSaBAU6Doh5UlPqBrZj1:0/consumer'
      var Model = function(item) {
        extendObservable(this, {
          loading: true,
          _deleted: false,
    ...
    it('should get item collection with object-based primary keys and item_value', done => {
      ...
      autorun(r => {
        ...
        if (!modelCollection['loading']) {
          expect(
            modelCollection.length >= 1 && modelCollection.length <= 3
          ).toBe(true)
